...
$sql = "SELECT id, username, email, COALESCE(ban_points, 0) as ban_points, COALESCE(normal_points, 0) as normal_points 
        FROM user
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-subtract" onclick="subtractNormalPoint(<?= $row['id'] ?>)">-1</button>
                        <?php
                        
                        if ($row['ban_points'] == 1) {
                            echo '<button class="btn btn-unban" onclick="updateBanStatus(' . $row['id'] . ', 0)">Unban</button>';
                        } else {
                            echo '<button class="btn btn-ban" onclick="updateBanStatus(' . $row['id'] . ', 1)">Ban</button>';
                        }
                        ?>
                    </td>
                </tr>
